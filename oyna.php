...
<?php
    ...
    $sql     = "SELECT tbl_yorumlar.id,tbl_yorumlar.kullanici_id,oyun_id,referans_id,tarih,yorum,tbl_kullanicilar.k_adi,tbl_kullanicilar.mail FROM tbl_yorumlar INNER join tbl_kullanicilar on tbl_yorumlar.kullanici_id=tbl_kullanicilar.id WHERE oyun_id=$gelenId and referans_id=0 order by tarih asc";
    $sorgula = mysqli_query($vt, $sql);
    while ($result = mysqli_fetch_array($sorgula)) {
        extract($result);
        ...
        echo "
      	...
      	<p><span class='user'>$k_adi</span><span class='date'>($tarih)</span></p><p class='text'>$yorum</p>";
        $kullanici_id = $_SESSION['id'];
            ...
            echo "<button type='button' style='border:none; background-color:transparent;' data-toggle='modal' data-target='#myModal'><a class='sayac'><img src='./images/like.png'>$sayac</a></button>
...
function reccoment($sorgulanacak_id, $a)
{
    ...
    $sql      = "SELECT tbl_yorumlar.id,tbl_yorumlar.kullanici_id,oyun_id,referans_id,tarih,yorum,tbl_kullanicilar.k_adi,tbl_kullanicilar.mail FROM tbl_yorumlar INNER join tbl_kullanicilar on tbl_yorumlar.kullanici_id=tbl_kullanicilar.id WHERE referans_id=$sorgulanacak_id order by tarih asc";
...
<div id='open-modal' class='modal-window'>
  <div>
    ...
    <h1>Beğenenler</h1>
    <div>";
    $sql= "SELECT 'tbl_kullanicilar.id','tbl_yorumbegeni.kullanici_id','tbl_yorumbegeni.yorum_id','tbl_kullanicilar.k_adi','tbl_kullanicilar.mail' FROM tbl_yorumbegeni INNER join tbl_kullanicilar on 'tbl_yorumbegeni.kullanici_id'='tbl_kullanicilar.id' WHERE 'yorum_id'=$id";
    $sorgula3 = mysqli_query($vt, $sql);
     while ($result3 = mysqli_fetch_array($sorgula3)) {
    ...
    echo "</div>
         ...
         <a class='yyorum' style='padding:8px;'>Yanıtla</a><div id='yyorum'style='display:none;'>
		<hr class='exampleone'>
      	<form action='' method='post' id='comment_form'>
      	<input type='hidden' name='r_id' value='$id'>
        <div id='bb_code'><p><img src='http://www.enguzeloyunlar.org/images/happy.gif' alt=':)' title=':)' class='bb_emoticon'><img src='http://www.enguzeloyunlar.org/images/sad.gif' alt=':(' title=':(' class='bb_emoticon'><img src='http://www.enguzeloyunlar.org/images/wink.gif' alt=';)' title=';)' class='bb_emoticon'><img src='http://www.enguzeloyunlar.org/images/@.gif' alt=':@' title=':@' class='bb_emoticon'><img src='http://www.enguzeloyunlar.org/images/cool.gif' alt=':8' title=':8' class='bb_emoticon'><img src='http://www.enguzeloyunlar.org/images/wave.gif' alt=':wave:' title=':wave:' class='bb_emoticon'><img src='http://www.enguzeloyunlar.org/images/think.gif' alt=':think:' title=':think:' class='bb_emoticon'><img src='http://www.enguzeloyunlar.org/images/clap.gif' alt=':clap:' title=':clap:' class='bb_emoticon'></p></div>
